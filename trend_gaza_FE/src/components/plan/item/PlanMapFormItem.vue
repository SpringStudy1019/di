...
const selectFunc = (data) => {
  ...
  } else if (allSelect.value[curDay.value].length >= 10) {
    // 각 N일차는 최대 10개만 등록 가능
...
const modifyPlansFunc = () => {
  modifyPlan(
    planIdx,
    trasformRequestDTO(),
    ({ data }) => {
      window.alert("여행 계획이 수정되었습니다.");
      router.push({ name: "plan-list" });
    },
    (error) => {
      console.log(error);
    ...
    >
      ...
      <div class="offcanvas-body">
        ...
        <div
          ...
          v-for="(selectItem, idx) in allSelect[curDay]"
          :key="selectItem.contentId"
        >
