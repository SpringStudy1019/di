...
class AttractionControllerTest {
	...
	void testSearchAttractions1() throws Exception {
		...
		mockMvc.perform(get("/attractions/search?sido=32&contentTypeId=&keyword=")).andExpect(status().isOk())
	...
	void testSearchAttractions2() throws Exception {
		...
		mockMvc.perform(get("/attractions/search?sido=32&contentTypeId=12&keyword=")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON)).andDo(print());
	...
	void testSearchAttractions3() throws Exception {
		...
		mockMvc.perform(get("/attractions/search?sido=32&contentTypeId=12&keyword=국립")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON)).andDo(print());
	...
	void testGetAttractionPlans() throws Exception {
		...
		mockMvc.perform(get("/attractions/plans?pgno=2")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON)).andDo(print());
	...
	void testGetParticipantsBookmarks() throws Exception {
		...
		mockMvc.perform(get("/attractions/plan/" + planIdx + "/bookmark")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON)).andDo(print());
	}
}
