...
class UserControllerTest {
	...
	void testRegister() throws Exception {
		...
		RegisterRequest registerRequest = new RegisterRequest("user04", "홍싸피", "1234", "01055553333", "user04",
				"naver.com", "MALE");
		...
		mockMvc.perform(post("/user/register").content(content).contentType(MediaType.APPLICATION_JSON))
	...
	void testIdCheck() throws Exception {
		...
		mockMvc.perform(get("/user/idCheck/{userId}", userId2)).andExpect(status().is2xxSuccessful())
				// .andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andDo(print());
	...
	void testLogin() throws Exception {
		...
		mockMvc.perform(post("/user/login").content(content).contentType(MediaType.APPLICATION_JSON))
	...
	void testFindId() throws Exception {
		...
		mockMvc.perform(post("/user/findId").content(content).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().is2xxSuccessful()).andExpect(content().contentType("text/plain;charset=UTF-8"))
				.andDo(print());
	}
	...
	void testFindPwd() throws Exception {
		...
		mockMvc.perform(post("/user/findPwd").content(content).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().is2xxSuccessful()).andExpect(content().contentType("text/plain;charset=UTF-8"))
				.andDo(print());
	}
	...
	void testView() throws Exception {
		...
		mockMvc.perform(get("/user/view/{userId}", userId)).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
	...
	void testModify() throws Exception {
		ModifyRequest modifyRequest = new ModifyRequest("user04", "홍싸피", "01077773333", "user04", "ssafy.com", "MALE");
		ObjectMapper objectMapper = new ObjectMapper();
		String content = objectMapper.writeValueAsString(modifyRequest);

		mockMvc.perform(put("/user/modify").content(content).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().is2xxSuccessful()).andDo(print());
	}
	...
	void testChangePwd() throws Exception {
		...
		mockMvc.perform(put("/user/changePwd").content(content).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().is2xxSuccessful()).andDo(print());
	}
	...
	void testUserDelete() throws Exception {
		...
		mockMvc.perform(delete("/user").param("userId", userId)).andExpect(status().isOk()).andDo(print());
	}
